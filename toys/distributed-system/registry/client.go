...
func RegisterService(r Registration) error {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to register service. registry service responded with status code %v", res.StatusCode)
	}
...
func UnregisterService(url string) error {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to unregister service. registry service responded with status code %v", res.StatusCode)
	}
...
func (suh serviceUpdateHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("update received %v]n", p)
	prov.Update(p)
}
