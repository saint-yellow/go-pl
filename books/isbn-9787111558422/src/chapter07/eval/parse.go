...
func Parse(input string) (_ Expression, err error) {
	defer func() {
...
func parseExpression(lex *lexer) Expression { return parseBinary(lex, 1) }

// binary = unary ('+' binary)*
// parseBinary stops when it encounters an
// operator of lower precedence that perc1.
func parseBinary(lex *lexer, prec1 int) Expression {
	lhs := parseUnary(lex)
...
// unary = '+' expr | primary
func parseUnary(lex *lexer) Expression {
...
//         | num
//         | '(' expr ')'
func parsePrimary(lex *lexer) Expression {
	switch lex.token {
	case scanner.Ident:
		id := lex.text()
		...
		if lex.token != ')' {
			for {
				args = append(args, parseExpression(lex))
				if lex.token != ',' {
					break
				}
				lex.next() // consume ','
